...
var loop = function () {
    ...
    // Handle rocket movement towards or away from the cursor with W and S keys
    var moveDirection = normalizeDirection(mousePosition.x - position.x, mousePosition.y - position.y);
    if (isWPressed) {
        position.x += moveDirection.x * 0.01; // Adjust speed as necessary
        position.y += moveDirection.y * 0.01;
    } else if (isSPressed) {
        position.x -= moveDirection.x * 0.01;
        position.y -= moveDirection.y * 0.01;
    }
    ...
    if (isSpacePressed && !projectile.active) {
        ...
        projectile.direction = normalizeDirection(mousePosition.x - position.x, mousePosition.y - position.y);
    }
...
function drawRocket() {
    // Set transformation matrix for the rocket
    ...
    // Bind the rocket buffer and draw
    gl.bindBuffer(gl.ARRAY_BUFFER, rectVBO);
...
function updateAndDrawProjectile() {
    if (projectile.active) {
        // Calculate the new end point of the projectile line
        // It starts from the rocket's current position and extends in the direction towards the mouse
        var lineEndPointX = projectile.position.x + projectile.direction.x * 0.3; // Length of the line
        var lineEndPointY = projectile.position.y + projectile.direction.y * 0.3;

        // Update the projectile's position to move it forward
        ...
        // Define vertices for the projectile line
        var projectileVertices = new Float32Array([
            projectile.position.x, projectile.position.y, 0.0, // Start point of the line
            lineEndPointX, lineEndPointY, 0.0 // End point of the line
        ]);

        // Create and bind the buffer for the projectile line
        var projectileBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, projectileBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, projectileVertices, gl.STATIC_DRAW);

        // Use the same attribute locations and draw the line
        gl.vertexAttribPointer(positionAttribLocation, 3, gl.FLOAT, false, 0, 0);
        gl.enableVertexAttribArray(positionAttribLocation);

        // Draw the projectile line
        gl.drawArrays(gl.LINES, 0, 2);

        // Optionally, add logic to deactivate the projectile if it goes off-screen or after a certain distance
        if (Math.abs(projectile.position.x) > 1 || Math.abs(projectile.position.y) > 1) {
